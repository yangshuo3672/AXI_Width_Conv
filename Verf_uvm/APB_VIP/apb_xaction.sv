...
class apb_xaction extends stb_rw_sequence_item; //vip connect with 9-step sequence

     rand logic [`APB_ADDR_WIDTH - 1 :0] addr;//Modify from reg to logic for jira373
     rand logic [`APB_DATA_WIDTH - 1 :0] data;//Modify from reg to logic for jira373
...
rand bit [`HISI_VIP_APB_MPBYPASS_PORT_WIDTH -1:0] m_bvMpubypass;
...
rand bit compare_mpubypass;//indicate the mpbypass needs to be compared or not ,in SocChecker
...
`uvm_object_utils_begin(apb_xaction)
  ...
  `uvm_field_int(m_bvAusser, UVM_ALL_ON)
...
constraint seq_need_resp_cons {
    soft this.seq_need_resp == 0;
}

constraint reasonable_pready_delay
{
    pready_delay == 'h0;
}
constraint idle_num_con
{
    idle_num == 'h0;
}
