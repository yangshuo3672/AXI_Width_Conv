`ifndef TC_OUTSTANDING_SV
`define TC_OUTSTANDING_SV

`define tc_name tc_outstanding
...
class `axi2axi_env_cfg(`tc_name) extends axi2axi_env_cfg;
  ...
  function void post_randomize();
    ...
    axi_mst_if_agent_cfg[0].out_of_bresp = 0;
    axi_mst_if_agent_cfg[0].out_of_rresp = 0;
    axi_mst_if_agent_cfg[0].rdintrlv_depth = 1;
    ...
    axi_mst_if_agent_cfg[0].mst_drv_cfg.ainfo_hold_when_invalid = axi_dec::VMT_TRUE;
    ...
    axi_slv_if_agent_cfg[0].out_of_bresp = 0;
    axi_slv_if_agent_cfg[0].out_of_rresp = 0;
    axi_slv_if_agent_cfg[0].rdintrlv_depth = 1;
...
function void `tc_name::build_phase(uvm_phase phase);
  ...
  if (!this.axi2axi_env_cfg.randomize()) begin
    `uvm_fatal(get_type_name(), "build_phase(): Unabled to randomize cfg in testcase.");
  end
  ...
  if (!this.my_axi_drv_cb.randomize()) begin
    `uvm_fatal(get_type_name(), "build_phase(): Unabled to randomize my_axi_drv_cb in testcase.");
  end
...
my_axi_slv_drv_cb.write_bvalid_delay = 500;
my_axi_slv_drv_cb.address_rvalid_delay = 500;
...
endtask:configure_phase
    ...
    int wr_cnt;
    int rd_cnt;
    bit wr_outstanding_reached;
    bit rd_outstanding_reached;
    ...
    wr_cnt = 0;
    rd_cnt = 0;
    wr_outstanding_reached = 0;
    rd_outstanding_reached = 0;

    fork
        while(1) begin
            @(posedge harness.u_ktp.aclk_s);
            if(harness.u_ktp.awvalid_s&harness.u_ktp.awready_s)begin
                wr_cnt++;
            end
            if(harness.u_ktp.bvalid_s&harness.u_ktp.bready_s)begin
                wr_cnt--;
            end
            if(harness.u_ktp.arvalid_s&harness.u_ktp.arready_s)begin
                rd_cnt++;
            end
            if(harness.u_ktp.rvalid_s&harness.u_ktp.rready_s)begin
                rd_cnt--;
            end
        end
        while(1) begin
            @(posedge harness.u_ktp.aclk_s);
            if(wr_cnt>16)begin
                `uvm_error(get_type_name(),$sformatf("outstanding of write exceeded 16"));
            end
            if(rd_cnt>16)begin
                `uvm_error(get_type_name(),$sformatf("outstanding of reed exceeded 16"));
            end
        end
  if(wr_cnt==16)begin
    wr_outstanding_reached = 1;
end
if(rd_cnt==16)begin
    rd_outstanding_reached = 1;
end
end
...
//void '(ktp_cstr.randomize());
//u_axi_seq.axi_write(env.axi_mst_if_agent[0].sqr, ktp_cstr.id, 1, ktp_cstr.length, 4, ktp_cstr.addr, 0, 0, 0, 0, wdata, w_strb, 0, 0);
//u_axi_seq.axi_read(env.axi_mst_if_agent[0].sqr, ktp_cstr.id, 1, ktp_cstr.length, 4, ktp_cstr.addr, 0, 0, 0, 0, rdata, 0, 0);
u_axi_seq.axi_write(env.axi_mst_if_agent[0].sqr, ktp_cstr.id, 1, ktp_cstr.length, 4, ktp_cstr.addr, ktp_cstr.awcache, ktp_cstr.awprot, 0, ktp_cstr.awqos, ktp_cstr.awuser, wdata, w_strb, 0, 0);
u_axi_seq.axi_read(env.axi_mst_if_agent[0].sqr, ktp_cstr.id, 1, ktp_cstr.length, 4, ktp_cstr.addr, ktp_cstr.arcache, ktp_cstr.arprot, 0, ktp_cstr.arqos, ktp_cstr.aruser, rdata, 0, 0);
join_none
end
join_none
#100us;
if(wr_outstanding_reached == 0) begin
`uvm_error(get_type_name(),$sformatf("outstanding of write never reachn 16"));
end
if(rd_outstanding_reached == 0) begin
`uvm_error(get_type_name(),$sformatf("outstanding of read never reachn 16"));
end
phase.drop_objection(this);
`uvm_info(get_type_name(), "main_phase(): main_phase finished", UVM_HIGH);
#100us;
wait((env.checker_inst.sw_data_q.size() == 0) && (env.checker_inst.sr_data_q.size() == 0));
phase.drop_objection(this);
`uvm_info(get_type_name(), "main_phase(): main_phase finished", UVM_HIGH);
endtask: main_phase
