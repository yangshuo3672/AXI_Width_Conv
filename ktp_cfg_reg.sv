...
module ktp_cfg_reg #(
  ...
  localparam ADDR_ID_BUFFER_DEPTH = 16;
  ...
  always_ff@(posedge clk or negedge resetn)begin
    ...
    else if(  |(irpt_raw_q & ~irpt_msk_q) )begin
      ktp_irpt_ns <= 1'b1;
    end
    else begin
      ktp_irpt_ns <= 1'b0;
  ...
  logic [ID_WIDTH+ADDR_WIDTH+1-1 : 0 ] aw_addr_id_table [ADDR_ID_BUFFER_DEPTH];
  logic [ID_WIDTH+ADDR_WIDTH+1-1 : 0 ] ar_addr_id_table [ADDR_ID_BUFFER_DEPTH];
  ...
  always_ff @(posedge clk or negedge resetn) begin
    if (!resetn) begin
      ...
      err_rd_id_q   <= '0;
      err_wr_id_q   <= '0;
      for (int i = 0; i < ADDR_ID_BUFFER_DEPTH; i++) begin
        aw_addr_id_table[i] <= '0;
        ar_addr_id_table[i] <= '0;
      end
    end else begin
      if (apb_valid_write) begin
        unique case (paddr)
...
//restore the every beat cmd addr/id in invalid table and set the table valid
   if(aw_fire)begin
       for(int aw_index = 0;aw_index < ADDR_ID_BUFFER_DEPTH;aw_index++)begin
           if( aw_addr_id_table[aw_index][40] == 0)begin   //valid
               aw_addr_id_table[aw_index]     <= {1'b1,aw_id,aw_addr};
               break;                                        //only detect one free buffer,after push id/addr to buffer,set buffer valid:bit[40]<=1'b1;
           end
       end
   end

  if(ar_fire)begin
      for(int ar_index = 0;ar_index < ADDR_ID_BUFFER_DEPTH;ar_index++)begin
          if( ar_addr_id_table[ar_index][40] == 0)begin
              ar_addr_id_table[ar_index]     <= {1'b1,ar_id,ar_addr};
              break;
          end
      end
  end

  //when error response;look up the addr/id in the valid table and set the table invalid
  if (b_error_fire && !irpt_raw_q[1]) begin          //if b channel return error && current no write error interrupt
      for(int aw_index=0;aw_index < ADDR_ID_BUFFER_DEPTH;aw_index++)begin
          if(( b_id == aw_addr_id_table[aw_index][39:32] ) && ( aw_addr_id_table[aw_index][40] ==1'b1 ))begin
              err_wr_addr_q     <= aw_addr_id_table[aw_index][31:0];
              err_wr_id_q       <= b_id;
              aw_addr_id_table[aw_index][40] <= 1'b0;   //set invalid
          end
      end
  end

  if (r_error_fire && !irpt_raw_q[0]) begin          //if r channel return error && current no read error interrupt
      for(int ar_index=0;ar_index < ADDR_ID_BUFFER_DEPTH;ar_index++)begin
          if(( r_id == ar_addr_id_table[ar_index][39:32] ) && ( ar_addr_id_table[ar_index][40]==1'b1 ))begin
              err_rd_addr_q     <= ar_addr_id_table[ar_index][31:0];
              err_rd_id_q       <= r_id;
              ar_addr_id_table[ar_index][40] <= 1'b0;
          end
      end
  end
      
//when correct response: also set the table invalid
  if( b_fire )begin
      for(int i=0; i < ADDR_ID_BUFFER_DEPTH;i++)begin
          if( ( b_id == aw_addr_id_table[i][39:32] ) && ( aw_addr_id_table[i][40] == 1'b1 ) )begin
              aw_addr_id_table[i][40] <= 1'b0;
          end
      end
  end
  ...
  if( r_fire && r_last )begin
      for(int i=0; i < ADDR_ID_BUFFER_DEPTH;i++)begin
          if( ( r_id == ar_addr_id_table[i][39:32] ) && ( ar_addr_id_table[i][40] == 1'b1 ) )begin
              ar_addr_id_table[i][40] <= 1'b0;
          end
      end
  end

   irpt_raw_q[1] <= (irpt_raw_q[1] && !irpt_clr_pulse[1]) || b_error_fire; //clear old error interrupt or generate new error interrupt
