验证工程师需要根据Spec对设计规格有自己深入且独立的理解，通过各种方法核对设计规范和设计代码的一致性，确保设计能够完成预定的功能。

（一）验证策略
    （1）UT（Unit Test）单元验证：
             验证对象为IP或模块，由IP或模块设计责任人承担；是自研IP的基础环节和关键验证环节。
             代码覆盖率/功能覆盖率需达到100%，UT的验证环境需重用到IT/ST。
    （2）IT（Intergrated Test）：
              验证对象为子系统或集成多个IP的大IP，由子系统交付责任人或验证责任人负责验证，如DDR子系统，CPU子系统，显示子系统，ISP子系统，USB、UFS、DP等；
              是最关键的IP/子系统的功能保障环节。代码覆盖率可以选择性覆盖，功能覆盖率需达到100%。
    （3）AIT（Architecture Intergrated Test）架构集成验证：
             验证对象为全芯片，由SOC验证组承担，关注总线架构功能、性能、鲁棒性、集成互联（中断、DMA、IOC等）的验证；
             是全芯片总线和集成验证的关键环节，其中在总线特性、总线死锁、QoS机制等方面需要将所有子系统展开进行验证。
    （4）ST（System Test）系统验证：
             验证对象为全芯片，由SOC验证组承担，重点关注全系统功能、全系统启动流程、低功耗流程、多子系统间的功能联合仿真。
             ST除处理器和低功耗验证使用真实处理器外，其他IP和子系统的验证使用VIP替换处理器。
...
2. 结合项目说明测试点是怎么分解的？什么流程步骤？
    ...
    分解要求：直观形象无歧义语言描述某一个功能点、保证不可细分、含义明确、分解完备、描述规范、划优先级

    需求规格--->Feature验证特性--->测试点--->测试用例

   （1）确定验证目标，验什么：功能，性能，接口，安全性    不验什么：避免范围蔓延
   （2）收集确认输入证据：包括需求规格、设计文档、接口协议
   （3）建立需求和功能分解结构：把系统按模块，子功能拆解，形成可验证的最小对象  SoC级或者是子系统级或者是IP级？
   （4）形成测试点清单：编号，名称，来源需求，测试目标，前置条件，预期结果
    

3. 讲解印象最深的一个测试用例？debug经历？有遇到什么coner的场景吗？
